import { PrismaClient, Gender, MaritalStatus, MembershipTier, VerificationStatus, ProfileStatus } from '@prisma/client';
...
const ALL_SCREENS = [
  ...
  { name: 'Member Dashboard',  slug: 'member-dashboard',    route: '/member/dashboard',          icon: 'LayoutDashboard', sortOrder: 20 },
...
const ROLE_SCREEN_ACCESS: Record<string, string[]> = {
  MEMBER: ['member-dashboard','profile-view','profile-edit','profile-complete','biodata-entry','profile-viewers','search','matches','messages','chat','interests','premium','payment-success','payment-history'],
  SUPPORT_AGENT: ['admin-dashboard','admin-users','admin-profiles','admin-reports','admin-logs'],
  MODERATOR: ['admin-dashboard','admin-users','admin-profiles','admin-biodata','admin-ai-biodata','admin-reports','admin-banners','admin-blogs','admin-communities','admin-success-stories','admin-testimonials','admin-faq','admin-static-pages','admin-logs'],
  ADMIN: ['admin-dashboard','admin-users','admin-profiles','admin-biodata','admin-ai-biodata','admin-plans','admin-payments','admin-reports','admin-banners','admin-blogs','admin-communities','admin-success-stories','admin-testimonials','admin-faq','admin-static-pages','admin-logs','admin-settings'],
  SUPER_ADMIN: [],
};

async function seedScreensAndRbac(db: PrismaClient) {
  console.log('🌱 Seeding screens...');
  for (const s of ALL_SCREENS) {
    await db.screen.upsert({ where: { slug: s.slug }, create: s, update: {} });
  }
  console.log(`  ✅ ${ALL_SCREENS.length} screens seeded`);

  console.log('🌱 Seeding screen-role permissions...');
  for (const [roleName, screenSlugs] of Object.entries(ROLE_SCREEN_ACCESS)) {
    ...
    for (const slug of screenSlugs) {
      const screen = await db.screen.findUnique({ where: { slug } });
      if (!screen) continue;
      await db.screenPermission.upsert({
...
async function main() {
  ...
  // 1b. Seed Modules & Actions (UAM Architecture)
  const modulesList = [
  ...
  // Helper to attach permissions to roles
  const assignPermissions = async (roleId: string, permKeys: string[]) => {
  ...
  // 4. Seed Communities, Sub-Communities, Castes & Sub-Castes
  ...
  for (const comm of ALL_COMMUNITIES) {
    ...
    // Determine religion ID based on slug
    ...
    // Seed Sub-Communities (child Communities) & Sub-Castes
  ...
  // 5. Seed Super Admin & Admin Users
  ...
  const existingSA = await prisma.userAssignment.findFirst({ where: { userId: superAdminUser.id, roleId: superAdminRole.id, communityId: null } });
  if (!existingSA) await prisma.userAssignment.create({ data: { userId: superAdminUser.id, roleId: superAdminRole.id } });
  ...
  const existingAdmin = await prisma.userAssignment.findFirst({ where: { userId: adminUser.id, roleId: adminRole.id, communityId: null } });
  if (!existingAdmin) await prisma.userAssignment.create({ data: { userId: adminUser.id, roleId: adminRole.id } });

  console.log('✅ Super Admin & Admin users seeded');

  // ── SCREENS & SCREEN PERMISSIONS ──────────────────────────
  await seedScreensAndRbac(prisma);
